...
export default function SignupPage() {
  ...
  const validate = () => {
    ...
    else if (passwordStrength < 3) e.password = 'Password is too weak';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) setErrors({ submit: result.error.message || 'Failed to create account.' });
      else router.push(redirectTo);
    } catch (err: any) { setErrors({ submit: err.message || 'Failed to create account.' }); }
      ...
      <main className="relative z-10 min-h-screen flex items-center justify-center pt-24 pb-16 px-6">
        <div className="w-full max-w-md">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <AuthInput id="password" type="password" label="Password" placeholder="Create a strong password"
                  value={formData.password} onChange={(e) => { setFormData({ ...formData, password: e.target.value }); setPasswordStrength(calcStrength(e.target.value)); }}
            ...
            <div className="mt-6">
              <SocialLogin
                onGoogleLogin={async () => { setLoading(true); try { await signInWithGoogle(redirectTo); } catch(e: any){ setErrors({submit: e.message}); } finally{ setLoading(false); }}}
                onFacebookLogin={async () => { setLoading(true); try { await signInWithFacebook(redirectTo); } catch(e: any){ setErrors({submit: e.message}); } finally{ setLoading(false); }}}
                loading={loading}
              />
            </div>
