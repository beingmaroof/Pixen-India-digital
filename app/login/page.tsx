...
// ─── Email-existence check via users table ───────────────────────────────────
async function checkEmailExists(email: string): Promise<boolean> {
  try {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) return false; // If query fails, assume unknown — let Supabase handle it
    return !!data;
  } catch {
    return false;
  }
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <button 
        onClick={() => router.back()}
        type="button"
        className="inline-flex items-center gap-2 text-white/60 hover:text-white transition-colors mb-6 text-sm font-medium group"
      >
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8">
        ...
        {/* Error banner */}
        {errors.submit && (
          <div className={`mb-6 p-4 rounded-xl border flex flex-col gap-2 ${errorType === 'no_account' ? 'bg-amber-500/10 border-amber-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
            <div className="flex items-start gap-3">
              <svg className={`w-5 h-5 flex-shrink-0 mt-0.5 ${errorType === 'no_account' ? 'text-amber-400' : 'text-red-400'}`} fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
              </svg>
              <p className={`text-sm ${errorType === 'no_account' ? 'text-amber-300' : 'text-red-300'}`}>{errors.submit}</p>
            </div>
            {/* Context-aware action buttons */}
            {errorType === 'no_account' && (
              <Link
                href={`/signup?email=${encodeURIComponent(formData.email)}`}
                className="mt-1 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-500/20 border border-amber-500/30 text-amber-200 text-sm font-semibold hover:bg-amber-500/30 transition-colors self-start"
              >
                Create Account →
              </Link>
            )}
            {errorType === 'wrong_password' && (
              <Link
                ...
                className="mt-1 inline-flex items-center gap-2 text-sm font-semibold text-purple-400 hover:text-purple-300 transition-colors"
              >
                Reset your password →
              </Link>
            )}
