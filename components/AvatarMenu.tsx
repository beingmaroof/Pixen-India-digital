...
export default function AvatarMenu() {
  ...
  const handleLogout = async () => {
    try {
      ...
      setIsOpen(false);
      router.push('/login');
      router.refresh(); // Force refresh to update AuthContext and UI state
    }
  };
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {/* Dropdown Menu */}
      <div 
        className={`absolute right-0 mt-3 w-48 bg-black/60 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_0_30px_rgba(0,0,0,0.5)] py-2 z-50 transition-all duration-200 origin-top-right ${
      ...
      >
        ...
        <Link 
          ...
          className="block px-4 py-2 text-sm text-white/80 hover:bg-white/10 hover:text-primary-400 transition-colors flex items-center"
        >
